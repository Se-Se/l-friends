...
export function getStrMDW (date) {
  ...
  return (date.getMonth() + 1) + '.' + date.getDate() + ' ' + weeks[date.getDay() - 1]
}

export function isOverDay (date) {
  const ONE_DAY = 1000 * 60 * 60 * 24
  const d = new Date(date + ' GMT+9') - new Date().getTime()
...
export function getDateFormatYMDHM (date) {
  if (date) {
    ...
    const amFlg = hours < 12
    hours = amFlg ? hours : hours - 12
    hours = hours < 10 ? ('0' + hours) : hours
    let minutes = slaDate.getMinutes()
    minutes = minutes < 10 ? ('0' + minutes) : minutes
    return y + '년 ' + m + '월 ' + d + '일' + (amFlg ? ' 오전 ' : ' 오후 ') + hours + ':' + minutes
  }
}
