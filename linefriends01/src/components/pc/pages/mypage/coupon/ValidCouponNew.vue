  ...
  <section>
    <div class="table-list__coupon">
      <div class="table-list--header">
        <h2 class="table-list--header__title">{{ $t('mypage.coupon.valid.title') }}</h2>
      </div>
      <div class="old-css">
        <div class="filter_con">
          <div class="search">
            <input
              type="text"
               :placeholder="$t('mypage.coupon.valid.discount')"
              v-model="promotionCode"
              @keypress.enter.prevent="registerCoupon"
            >
            <span class="btn_search">
              <button class="btn_gray" @click.prevent="registerCoupon">{{ $t('mypage.coupon.valid.add') }}</button>
            </span>
          </div>
        </div>
      </div>
      <table class="table coupon-table">
        ...
        <thead>
          <tr>
            ...
            <th>{{ $t('mypage.coupon.list.validity') }}</th>
            <th>{{ $t('mypage.coupon.list.kinds') }}</th>
          </tr>
        </thead>
        <tbody v-if="getValidCoupons && getValidCoupons.totalCount > 0">
          <tr
            v-for="(item,index) in getValidCoupons" :key="index"
            class="coupon-tr"
          >
            <td>
