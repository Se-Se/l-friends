...
const apiStore = createNcpApiStore([
  {
    ...
    path: 'categories',
    onSuccess (state, res) {
      state.categories = res.data.multiLevelCategories
    },
    cacheable: true,
    cacheMaxAge: 60e3 * 60
  },
  {
    action: 'newProductCategoryFetch',
    property: 'news',
...
export default {
  ...
  mixins: [apiStore],
  actions: {
    async fetchForCate ({ state, dispatch, commit, rootState }, to) {
      await dispatch('fetchAll')
      await commit('SET_CATEGORY', to)
      /*
      await commit('CHECK_NO', to)
      await commit('SELECTED_SET', to)
      */
    }
  },
  getters: {
