...
import { isLogin, refreshCartStoragePeriod, checkCartStoragePeriod, formatOptionTitle, uniqueArr } from '@/utils/utils'
...
export default {
  ...
  state: {
    ...
    generalProd: { 'generalCart': [] },
    reservationProd: { 'reservationCart': [] },
    calculate: {
  ...
  mutations: {
    FORMAT_CART (state) {
      if (state.cart) {
        ...
        state.reservationProd.reservationCart.deliveryGroups = tempCart.deliveryGroups && tempCart.deliveryGroups.filter(delivery => {
        ...
        state.generalProd.generalCart.deliveryGroups = tempCart.deliveryGroups && tempCart.deliveryGroups.filter(delivery => {
          return delivery.orderProducts.filter(product => {
            return product.orderProductOptions.filter((option) => {
        ...
        state.generalProd.generalCart.deliveryGroups && state.generalProd.generalCart.deliveryGroups.forEach(delivery => {
          let tempDelivery = {
            deliveryCondition: delivery.deliveryCondition,
            deliveryPayType: delivery.deliveryPayType,
            buyAmt: delivery.buyAmt,
            deliveryAmt: delivery.deliveryAmt,
            items: []
          }
          delivery.orderProducts.forEach(product => {
            product.orderProductOptions.forEach(option => {
              tempDelivery.items.push({
                isInvalid: false,
          ...
          state.showItems.push(tempDelivery)
        })
        if (state.reservationProd.reservationCart && state.reservationProd.reservationCart.deliveryGroups) {
          let reservationYmdtArr = []
          state.reservationProd.reservationCart.deliveryGroups.forEach(delivery => {
            delivery.orderProducts.forEach(product => {
              product.orderProductOptions.forEach(option => {
          ...
          reservationYmdtArr.forEach((ymd) => {
            let temp = state.reservationProd.reservationCart.deliveryGroups.filter(delivery => {
              return delivery.orderProducts.filter(product => {
                return product.orderProductOptions.filter((option) => {
    ...
    CHECK_ALL (state, { checked }) {
      state.checkedItems = []
      if (checked) {
        state.showItems.forEach(delivery => {
    ...
    CHECK_OPTION (state, { checked, cartNo }) {
      if (state.showItems) {
        if (checked) {
          let tempItem = null
          state.showItems.forEach(delivery => {
            delivery.items.forEach(option => {
              if (Number(option.cartNo) === Number(cartNo)) {
                tempItem = option
              }
            })
          })
          if (tempItem) {
            state.checkedItems.push({
              cartNo: tempItem.cartNo,
              orderCnt: tempItem.orderCnt,
              productNo: tempItem.productNo,
              optionNo: tempItem.optionNo
            })
          }
        } else {
    ...
    CHECK_ALL_RVT (state, { index, checked }) {
      ...
      if (checked) {
        state.showReservationItems[index].forEach(delivery => {
          if (!delivery.isInvalid) {
            delivery.items.forEach(item => {
