...
export default {
  ...
  actions: {
    async fetchProductInquiry ({ state, dispatch, commit, rootState }, payload) {
      ...
      commit('SET_PARAMS', {
        ...
        pageNumber: payload.pageNo ? payload.pageNo : 1,
    ...
    async putProductInquiry ({ state, dispatch, commit, rootState }, updateparams) {
      await dispatch('_putProductInquiry', { data: updateparams.data, params: { inquiryNo: updateparams.inquiryNo } })
      await dispatch('fetchProductInquiry', {
  ...
  mutations: {
    RESET_LIST (state) {
      // state.inquiryList.item = null
      state.totalCount = null
      state.loading = true
      state.fetchListParams = defaultParams
    },
    RESET_INQUERY_ITEM (state) {
      state.inquiryList.item = null
    },
    RESET_DETAIL (state) {
      state.inquiry = null
