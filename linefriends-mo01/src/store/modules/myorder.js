...
export default {
  ...
  mixins: [apiStore],
  actions: {
    getOrder ({ state, dispatch }, { orderNo }) {
      state.order = null
      dispatch('_getOrder', { params: { orderNo } })
    },
    async editDeliveries ({ dispatch }, { orderNo, data }) {
      await dispatch('_editDeliveries', { params: { add: false, orderNo }, data })
    },
    async getOrderListInInquiry ({ state, commit, dispatch }, { type, period, to }) {
      commit('RESET_LIST')
      if (period === 'six_month') {
        commit('SET_PARAMS', {
    ...
    async fetchMoreOrderListInInquiry ({ state, dispatch, commit, rootState }, payload) {
      commit('SET_PARAMS', {
        pageNumber: payload
      })
      state.orders = []
